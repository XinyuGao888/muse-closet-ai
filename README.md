...
> Figure 1：人物照片已载入，云衣柜中的“炭灰色腰带长风衣”处于选中状态；绿色描边和可用的生成按钮让用户明确知道输入已经准备完成。
...
2. 使用自动生成的 6 件样衣，查看天气与场合驱动、最多 3 套且不重复凑数的推荐。
...
- 根据天气、场合、正式度、指定单品与偏好生成最多 3 套可解释搭配
- 搭配推荐先经过“连衣裙不与上下装混搭、核心单品场合兼容、冷暖层次、量感比例、色彩主次”硬校验，再结合收藏、穿着与反馈排序；不足 3 套时宁缺毋滥，不用重复组合凑数
- 自然语言穿搭入口与浏览器语音输入
...
当前版本没有把“看起来像达人”的图片直接当成训练数据，也不会声称已经自动学习小红书博主。它把公开时尚编辑内容中的可解释原则整理为产品规则，再映射到用户自己的衣柜：

- [Vogue：How to Style the Layered Look](https://www.vogue.com/article/addressed-how-to-style-the-layered-look)：提炼“先判断比例，再控制色盘与层次”的原则。
- [Vogue：How to Master Color Theory in Clothing](https://www.vogue.com/article/color-theory-for-clothing)：提炼中性色托底、单一强调色、同色系和邻近色关系。
- [Who What Wear：Coat and Dress Outfit Ideas](https://www.whowhatwear.com/coat-and-dress-outfit-ideas)：用于定义“裙装＋大衣不是禁搭，但必须满足场合、内外量感和纵向线条”的例外条件。

推荐链路是：先生成“连衣裙”或“上装＋下装”基础结构 → 用硬规则排除品类、场合、温度和比例冲突 → 对色彩、风格与偏好打分 → 用组合相似度去重。公开内容只用于规则归纳与产品研究，不抓取图片训练模型；后续若接入真实达人穿搭库，需要使用获得许可的数据，并增加人工审核和 Bad Case 标签。
