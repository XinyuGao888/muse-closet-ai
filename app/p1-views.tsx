...
export function OutfitDiary({ entries, insights, plans, busy, onSubmit }: { entries: DiaryEntry[]; insights: DiaryInsights | null; plans: OutfitPlan[]; busy: boolean; onSubmit: (form: FormData) => void }) {
  ...
  return <section className="diary-page"><header className="p1-page-hero"><div><p className="eyebrow">MY OUTFIT MEMORY</p><h1>我的穿搭记录</h1><p>保存实际穿着照片和感受，让 Muse 逐渐学会你真正喜欢、也真的会穿的搭配。</p></div></header>
    <div className="diary-insights"><article><span>穿搭记录</span><strong>{insights?.totalEntries ?? 0}</strong><small>篇</small></article><article><span>偏好松紧度</span><strong>{fitTop?.label ?? "待学习"}</strong><small>{fitTop ? `${fitTop.count} 次真实信号` : "记录后开始学习"}</small></article><article><span>平均舒适度</span><strong>{insights?.averageComfort || "—"}</strong><small>/ 5</small></article></div>
    <div className="diary-compose"><label className={preview ? "diary-photo has-image" : "diary-photo"}>{preview ? <img src={preview} alt="穿搭照片预览" /> : <><span>＋</span><strong>上传今天的穿搭照片</strong><small>镜子自拍或自然站姿都可以</small></>}<input type="file" accept="image/*" onChange={choose} /></label><div className="diary-form"><label>关联计划（可选）<select value={planId} onChange={(event) => setPlanId(event.target.value)}><option value="">不关联计划</option>{plans.slice(0, 30).map((item) => <option value={item.id} key={item.id}>{item.planDate} · {item.name}</option>)}</select></label><textarea placeholder="今天穿着感觉怎么样？（可选）" value={caption} onChange={(event) => setCaption(event.target.value)} /><details className="diary-advanced"><summary>补充穿着反馈（可选）</summary><div><div className="diary-fit"><span>实际松紧度</span>{["偏松", "合身", "偏紧"].map((item) => <button className={fit === item ? "is-active" : ""} onClick={() => setFit(item)} key={item}>{item}</button>)}</div><label>舒适度 <b>{comfort}/5</b><input type="range" min="1" max="5" value={comfort} onChange={(event) => setComfort(Number(event.target.value))} /></label><label>收到的好评次数<input type="number" min="0" max="99" value={compliments} onChange={(event) => setCompliments(Number(event.target.value))} /></label><textarea placeholder="和计划搭配相比，哪里不同？" value={difference} onChange={(event) => setDifference(event.target.value)} /></div></details><button className="primary-button" disabled={!photo || busy} onClick={submit}>{busy ? "正在保存…" : "保存穿搭记录"}</button></div></div>
